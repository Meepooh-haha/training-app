...
router.get('/gap-analysis', h((req, res) => {
...
router.get('/roadmap/:employee_code', h((req, res) => {
  res.json(db.prepare(`
    SELECT er.*,
  ...
  `).all(req.params.employee_code));
}));

router.post('/roadmap/:employee_code', h((req, res) => {
...
router.put('/roadmap/:employee_code/:id', h((req, res) => {
  ...
  `).run(status, target_quarter || '', course_code || null, notes || '',
         req.params.id, req.params.employee_code);
  res.json({ ok: true });
}));

router.delete('/roadmap/:employee_code/:id', h((req, res) => {
  db.prepare('DELETE FROM employee_roadmaps WHERE id=? AND employee_code=?')
    .run(req.params.id, req.params.employee_code);
  res.json({ ok: true });
}));
