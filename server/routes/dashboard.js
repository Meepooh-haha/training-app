...
// ---------------- Module 3: Dashboard stats ----------------
router.get('/dashboard/stats', h((req, res) => {
  const year = String(new Date().getFullYear());
  const reqs = db.prepare('SELECT * FROM training_requests').all();
  ...
  const avg = db.prepare('SELECT AVG(total_score) AS a FROM training_evaluations').get().a || 0;
